...
local dataProvider = MSLDataProvider
local sectorList = {}
...
local config = {
    ...
    buySellOffersCount = uiScale > 1.3 and 5 or 8, -- limit buy/sell offers for bigger UI scales
    ...
    sortResDropdownOptions = {
        ...
        { id = 2008, invert = false, resource = "rawscrap", text = ReadText(20811, 257), icon = "", displayremoveoption = false },
    ...
    dropdownHeight = Helper.standardTextHeight + 8,
...
function sectorList.register ()
    sectorList.debugText("REGISTER")

    mapMenu = Helper.getMenu("MapMenu")

    mapMenu.registerCallback("ic_onSelectElement", sectorList.onSelectElement)
    mapMenu.registerCallback("ic_onRowChanged", sectorList.onRowChanged)
    mapMenu.registerCallback("createRightBar_on_start", sectorList.createRightBar)
    mapMenu.registerCallback("createInfoFrame2_on_menu_infoModeRight", sectorList.createInfoFrame2_on_menu_infoModeRight)
    mapMenu.registerCallback("refreshInfoFrame2_on_start", sectorList.refreshInfoFrame2_on_start)
end

function sectorList.createRightBar(globalConfig)
    sectorList.debugText("Creating right bar")
    ...
    if not isMapSectorListExists then
        ...
        sectorList.debugText("Creating icon")
    end

    mapMenu.prepareEconomyWares()
end

function sectorList.refreshInfoFrame2_on_start()
...
function sectorList.createInfoFrame2_on_menu_infoModeRight (infoFrame2)

    if mapMenu.searchTableMode == config.mode then

        if (shouldUpdate) then
            sectorList.debugText("Providing data")
            data = dataProvider.prepareData()
            updatedAt = C.GetCurrentGameTime()
            shouldUpdate = false
        end

        sectorList.debugText("Drawing MSL", mapMenu.searchTableMode)

        local refreshButtonWidth = Helper.scaleY(17)

        -- settings table
        local settingsTable = infoFrame2:addTable(config.settingsTable.columnsNumber, { tabOrder = 1, highlightMode = "off", skipTabChange = true, multiSelect = false, backgroundID = "solid", backgroundColor = Helper.color.semitransparent, reserveScrollBar = false })
        settingsTable:setColWidth(5, refreshButtonWidth)
        ...
        local lastUpdatedTextProperties = sectorList.shallowCopy(config.sortHederProperties)
        ...
        local lastUpdatedTimeProperties = sectorList.shallowCopy(config.sortHederProperties)
        lastUpdatedTimeProperties.color = { r = 140, g = 140, b = 140, a = 100 }
        local timeUpdatedText = sectorList.getTimeUpdatedText()
        row[4]:createText(timeUpdatedText .. " " .. ReadText(20811, 151), lastUpdatedTimeProperties)
        row[5]:createButton({ mouseOverText = ReadText(20811, 141) }):setIcon("msl_update", { })
        row[5].handlers.onClick = function()
            sectorList.refreshData()
        end
        ...
        row[1]:createDropDown(config.sortPropDropdownOptions, { height = config.dropdownHeight, startOption = sort.typeId })
        row[1].handlers.onDropDownActivated = function()
            mapMenu.noupdate = true
        end
        row[1].handlers.onDropDownConfirmed = function(_, id)
            return sectorList.onPropDropDownConfirmed(id, config.sortPropDropdownOptions)
        end

        row[2]:createDropDown(config.sortResDropdownOptions, {height = config.dropdownHeight, startOption = sort.typeId })
        row[2].handlers.onDropDownActivated = function()
            mapMenu.noupdate = true
        end
        row[2].handlers.onDropDownConfirmed = function(_, id)
            return sectorList.onResDropDownConfirmed(id, config.sortResDropdownOptions)
        end
        ...
        table.sort(buyDropdownOptions, function(a, b)
            return sectorList.propertySorter(a, b, 'text', true)
        end)

        row[3]:createDropDown(buyDropdownOptions, { height = config.dropdownHeight, startOption = sort.typeId })
        row[3].handlers.onDropDownActivated = function()
            mapMenu.noupdate = true
        end
        row[3].handlers.onDropDownConfirmed = function(_, id)
            return sectorList.onBuySellDropDownConfirmed(id, buyDropdownOptions, 'offersBuy')
        end
        ...
        row[4]:setColSpan(2):createDropDown(sellDropdownOptions, { height = config.dropdownHeight, startOption = sort.typeId })
        row[4].handlers.onDropDownActivated = function()
            mapMenu.noupdate = true
        end
        row[4].handlers.onDropDownConfirmed = function(_, id)
            return sectorList.onBuySellDropDownConfirmed(id, sellDropdownOptions, 'offersSell')
        end
        ...
        -- sectors table
        local sectorsTable = infoFrame2:addTable(config.sectorsTable.columnsNumber, {
            tabOrder = 2,
            borderEnabled = true,
            backgroundID = "solid",
            backgroundColor = Helper.color.semitransparent,
            y = settingsTable.properties.y + settingsTable:getVisibleHeight() + Helper.borderSize,
            reserveScrollBar = true
        })
        sectorsTable:setColWidth(1, Helper.scaleX(20), false) -- expand btn
        sectorsTable:setColWidth(2, Helper.scaleX(50), false) -- faction name
        sectorsTable:setColWidth(8, Helper.scaleX(30), false) -- center sector button
        ...
        for _, sector in ipairs(data) do
            ...
            local isSectorExpanded = sectorList.isSectorExpanded(tostring(sectorComponent))

            row[1]:createButton({ height = 25, bgColor = Helper.color.semitransparent }):setText(isSectorExpanded and "-" or "+", {
                halign = "center",
            })
            row[1].handlers.onClick = function()
                return sectorList.buttonExpandSector(tostring(sectorComponent))
            end

            local factionShortTextProperties = sectorList.shallowCopy(config.factionShortTextProperties)
            ...
            -- sector name
            local textProperties = sectorList.shallowCopy(config.textProperties)
            ...
            if sector.hasshipyard then
                stationIcons = sectorList.addIcon(stationIcons, config.icons.shipyard.icon)
            end
            if sector.haswharf then
                stationIcons = sectorList.addIcon(stationIcons, config.icons.wharf.icon)
            end
            if sector.hasequipmentdock then
                stationIcons = sectorList.addIcon(stationIcons, config.icons.equipmentdock.icon)
            end
            if sector.hasplayerhq then
                stationIcons = sectorList.addIcon(stationIcons, config.icons.playerhq.icon)
            end
            if sector.hasfactionhq then
                stationIcons = sectorList.addIcon(stationIcons, config.icons.factionhq.icon)
            end
            row[6]:setColSpan(3):createText(stationIcons, { halign = "right", y = 0, fontsize = 15, height = 15, titleColor = Helper.color.semitransparent })

            -- center map on sector button
            --[[row[8]:createButton({ y = Helper.scaleY(3), width = Helper.scaleX(10), mouseOverText = string.format(ReadText(20811, 142), ffi.string(sector.name)) }):setIcon("menu_center_selection", {
                y = 8,
            })
            ...
            if (isSectorExpanded) then
                ...
                if (sector.image ~= '') then
                    row[2]:setColSpan(7):createIcon(sector.image, { height = Helper.scaleY(150), x = 6, scaling = true })
                end
                ...
                -- owner
                sectorList.addSubRow(sectorsTable, ReadText(1001, 9040), sector.ownername)

                -- population
                sectorList.addSubRow(sectorsTable, ReadText(1001, 9041), ConvertIntegerString(tonumber(sector.population), true, 3, true))

                -- workforce Availability Bonus
                sectorList.addSubRow(sectorsTable, ReadText(1001, 11296), string.format("%+.0f%%", (sector.workforceAvailabilityBonus * 100)))

                -- station Workforce
                sectorList.addSubRow(sectorsTable, ReadText(1001, 2456), ConvertIntegerString(sector.stationWorkforce, true, 3, true))

                -- known Stations
                sectorList.addSubRow(sectorsTable, ReadText(1001, 9042), sector.knownStations)

                -- main Production
                sectorList.addSubRow(sectorsTable, ReadText(1001, 9050), sector.mainProduction)
                ...
                -- Sunlight
                sectorList.addSubRow(sectorsTable, ReadText(1001, 2412), sector.sunlight .. "%")

                for _, resourceEntry in ipairs(sector.resources) do
                    sectorList.addSubRow(sectorsTable, resourceEntry.name, ConvertIntegerString(resourceEntry.current, true, 3, true))
                end
                ...
                local textRight = sectorList.shallowCopy(config.rowTextProperties)
...
function sectorList.refreshData()
    shouldUpdate = true
    sort = {
        typeId = 1001
    }
    sectorList.resetStates()
end

function sectorList.getTimeUpdatedText()
    local minute = 60
...
function sectorList.addIcon(iconsString, icon)
    iconsString = iconsString .. " \27[" .. icon .. "]"
    return iconsString
end

function sectorList.onPropDropDownConfirmed(id, options)
    disabledSectors = {}
    sort.typeId = id
    ...
    for _, option in ipairs(options) do
        if (tostring(option.id) == id) then
            table.sort(data, function(a, b)
                return sectorList.propertySorter(a, b, option.property, option.invert)
            end)
        end
    end
    sectorList.resetStates()
...
function sectorList.onResDropDownConfirmed(id, options)
    disabledSectors = {}
    sort.typeId = id
    ...
    for _, option in ipairs(options) do
        if (tostring(option.id) == id) then
            table.sort(data, function(a, b)
                return sectorList.resourceSorter(a, b, option.resource, option.invert)
            end)
        end
    end
    sectorList.resetStates()
...
function sectorList.onBuySellDropDownConfirmed(id, options, type)
    disabledSectors = {}
    sort.typeId = id
    ...
    for _, option in ipairs(options) do
        if (tostring(option.id) == id) then
            table.sort(data, function(a, b)
                return sectorList.buySellOfferSorter(a, b, type, option.ware, option.invert)
            end)
        end
    end
    sectorList.resetStates()
...
function sectorList.propertySorter(sectorA, sectorB, propertyName, invert)
...
function sectorList.buySellOfferSorter(sectorA, sectorB, type, resourceName, invert)
    local ax = 0
    local bx = 0
...
function sectorList.resourceSorter(sectorA, sectorB, resourceWare, invert)

    local ax = 0
    local bx = 0

    for _, resource in ipairs(sectorA.resources) do
        if (resource.ware == resourceWare) then
            ax = resource.current
        end
    end
...
function sectorList.centerOnSector(sectorComponent)
    C.SetFocusMapComponent(mapMenu.holomap, sectorComponent, true)
end

function sectorList.isSectorExpanded(sector)
    return expandedSectors[sector] == true
end

function sectorList.buttonExpandSector(sector)
...
function sectorList.resetStates()
    expandedSectors = {}
...
function sectorList.onSelectElement(uitable, modified, row, isdblclick, input)
...
function sectorList.onRowChanged(row, rowdata, uitable, modified, input, source)
    if mapMenu.searchTableMode == config.mode then
        mapMenu.noupdate = false
    end
end

function sectorList.addSubRow(uitable, dataleft, dataright)
...
function sectorList.shallowCopy(original)
    local copy = {}
...
function sectorList.debugText(title, text)
    if (isDebug) then
        DebugError("sector_list.lua: " .. title .. ": " .. tostring(text))
    end
end
